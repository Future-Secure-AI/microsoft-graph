...
import getWorkbookPivotTable from "./getWorkbookPivotTable.ts";

const notFoundPattern = /The requested resource doesn't exist/i;
describe("getWorkbookPivotTable", () => {
	it("can retrieve a pivot table by its ID", async () => {
		...
		try {
			...
			await expect(() => getWorkbookPivotTable(pivotTableRef)).rejects.toThrowError(notFoundPattern);
		} finally {
