...
import unmergeWorkbookRange from "./unmergeWorkbookRange.ts";
import updateWorkbookRange from "./updateWorkbookRange.ts";

describe("unmergeWorkbookRange", () => {
	it("unmerges a range without error", { timeout: 20000 }, async () => {
		const values = [
			[1, 2],
			[3, 4],
		];
		...
		try {
			...
			await unmergeWorkbookRange(rangeRef);
		} finally {
