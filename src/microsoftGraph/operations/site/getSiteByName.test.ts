...
describe("getSiteByName", () => {
    it("can retrieve an existing site by name", async () => {
        const defaultSite = await executeSingle(getSite(defaultSiteRef));
    ...
    it("throws an error when trying to retrieve a non-existent site by name", async () => {
        ...
        await expect(executeSingle(getSiteByName(nonExistentHostName, nonExistentSiteName))).rejects.toThrow();
    });
});
