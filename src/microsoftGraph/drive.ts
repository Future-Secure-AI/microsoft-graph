 ...
 */
export const getItemByPath = async (
  driveRef: DriveRef,
  itemPath: ItemPath,
): Promise<DriveItem> =>
  apiGet<DriveItem>(`/sites/?/drives/?/root:${itemPath}`, [
    driveRef.site,
    driveRef.drive,
  ]);
 ...
 */
export const listItemChildenByPath = async (
  driveRef: DriveRef,
  itemPath: ItemPath,
): Promise<ListItemResponse> => {
  const output: DriveItem[] = [];

  let response = await apiGet<ListItemResponse>(
    `/sites/?/drives/?/root:${itemPath}:/children`,
    [driveRef.site, driveRef.drive],
  );
...
export const createFolder = async (
  ...
  apiPost<DriveItem>(
    `/sites/?/drives/?/root:${folderPath}:/children`,
    [driveRef.site, driveRef.drive],
    {
      name: folderPath,
      folder: {},
...
export const copyItem = async (
  srcFileRef: ItemRef,
  dstFolderRef: ItemRef,
  dstFileName: string,
): Promise<void> =>
  apiPost(
    "/sites/?/drives/?/items/?/copy",
    [srcFileRef.site, srcFileRef.drive, srcFileRef.item],
    {
      name: dstFileName,
      parentReference: {
